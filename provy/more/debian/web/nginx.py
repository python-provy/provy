...
<pre class="sh_python">
    class MySampleRole(Role):
        ...
        result = self.remote_symlink(self.__available_site_for(site), self.__enabled_site_for(site), sudo=True)
        if result:
...
<pre class="sh_python">
    class MySampleRole(Role):
        ...
        result = self.update_file(template, self.__available_site_for(site), options=options, sudo=True)
        if result:
